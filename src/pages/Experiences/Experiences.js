...
const Experiences = () => {
    ...
    return (
        <div className="font-sans text-slate-800 dark:text-slate-200 bg-page-bg selection:bg-accent/20 transition-colors duration-300">
            ...
            {/* Categories Bar */}
            <div className="bg-card-bg border-b border-card-border sticky top-20 z-40 transition-colors">
                <div className="container mx-auto px-6 overflow-x-auto">
                    <div className="flex items-center gap-8 py-4 whitespace-nowrap min-w-max">
                        {['All Experiences', 'Adventure', 'Nature', 'Wellness', 'Culture', 'Luxury'].map((cat, i) => (
                            <button
                                key={i}
                                className={`text-sm font-bold tracking-wide transition-colors ${i === 0 ? 'text-accent' : 'text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white'}`}
                            >
                                {cat}
                            </button>
                        ))}
                            ...
                            <FadeInSection key={i} delay={`${(i % 3) * 150}ms`}>
                                <div className="group cursor-pointer">
                                    ...
                                    <div className="flex justify-between items-center px-4">
                                        ...
                                        <button className="w-10 h-10 bg-slate-100 dark:bg-slate-800 dark:text-slate-400 rounded-full flex items-center justify-center group-hover:bg-accent dark:group-hover:bg-accent group-hover:text-white transition-all transform group-hover:translate-x-1">
                                            →
                                        </button>
                                    </div>
                                </div>
            ...
            {/* CTA Section */}
            <section className="py-24 bg-page-bg transition-colors">
                <div className="container mx-auto px-6 text-center">
                    <FadeInSection>
                        <div className="bg-accent/5 dark:bg-accent/10 rounded-[3rem] p-12 md:p-20 relative overflow-hidden transition-colors">
                            <div className="absolute top-0 right-0 -mr-20 -mt-20 w-80 h-80 bg-accent/20 dark:bg-accent/10 rounded-full blur-3xl" />
                            <div className="relative z-10">
                                ...
                                <button className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-10 py-4 rounded-full font-bold hover:bg-accent dark:hover:bg-accent/80 transition-all shadow-xl shadow-slate-900/20 dark:shadow-none">
